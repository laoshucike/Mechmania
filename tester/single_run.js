...
console.log(Object.keys(latestRanking).length)
  Object.keys(latestRanking).forEach(name => {
  })


name ='txu'

    var res = playForXTimes('reference', name, 3)
    console.log(res)
    if (res.first_win) {
      console.log('reference win above ' + name)
      win ++;
    } else {
      console.log('reference lose to ' + name)
      lose ++;
    }
